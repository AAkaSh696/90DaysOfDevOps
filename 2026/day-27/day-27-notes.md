## Day 27 – GitHub Profile Makeover: Build Your Developer Identity

## Task 1: Audit Your Current GitHub Profile
1. No , even i didn't set profile picture lol.
2. No, its empty now
3. It's relevant
4. No , its missing
5. Nope ! it needs strong improvement

## Task 2: Create Your Profile README
 Done Task 2
 
## Task 3: Organize Your Repositories
- Organized 90daysofdevops by day folders
- Organized shell-scripts repo
- Organized devops-notes repo

## Task 4: Pin Your Best Repos
Pinned best repo ( for right now which i have) 

## Task 5: Clean Up
- Delete empty repos
- Rename unclear repos

## Task 6: Before & After
- Improved whole github profile
- Organized Repos
- Removes unwanted repos 
